...
# This version includes the definitive fix for the 422 error on the dashboard.
#
# To Run This Backend:
...
@jwt_required()
def get_all_orders():
    # ## THIS IS THE CORRECTED FUNCTION THAT FIXES THE 422 ERROR ##
    try:
        conn = get_db_connection()
        ...
        for row in orders_cursor:
            orders.append({
                ...
                # Convert the datetime object to a standard ISO string for JSON
                "order_date": row["order_date"].isoformat() if isinstance(row["order_date"], datetime.datetime) else row["order_date"],
                "status": row["status"],
